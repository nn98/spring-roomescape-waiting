...
public class PaymentOrderService {
    ...
    public PaymentOrder confirm(PaymentOrder order, String name, Long sessionId, String paymentKey) {
        return paymentOrderRepository.save(order.confirmed(name, sessionId, paymentKey));
    }
    ...
    public List<PaymentOrder> findByName(String name) {
        return paymentOrderRepository.findByNameOrderByIdDesc(name);
    }
    ...
    public void recordUnknown(String orderId, String name, Long sessionId, String paymentKey) {
        paymentOrderRepository.save(getByOrderId(orderId).unknown(name, sessionId, paymentKey));
    }

    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public void recordFailed(String orderId, String name, Long sessionId) {
        paymentOrderRepository.save(getByOrderId(orderId).failed(name, sessionId));
    }

    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public void recordRetryable(String orderId, String name, Long sessionId) {
        paymentOrderRepository.save(getByOrderId(orderId).retryable(name, sessionId));
    }
}
